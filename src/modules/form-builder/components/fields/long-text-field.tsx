...
import { StyledTextArea } from "@/styled-components/styled-text-area"
...
export const LongTextField: React.FC<{
  ...
  const { id, label, required, placeholder, helper_text } =
    elementInstance as TLongTextField
...
export const LongTextFieldDesigner: React.FC<{
  elementInstance: FormElementInstance
}> = ({ elementInstance }) => {
  ...
  return (
    <div className="flex flex-col w-full gap-2">
      ...
      <StyledTextArea readOnly placeholder={placeholder} />
...
const propertiesSchema = ZLongTextField.omit({
  type: true,
  id: true,
})
type propertiesSchemaType = z.infer<typeof propertiesSchema>
export const LongTextFieldProperties: React.FC<{
  elementInstance: FormElementInstance
}> = ({ elementInstance }) => {
  const { updateElement } = useFormBuilderStore((state) => state)
  const element = elementInstance as TLongTextField
