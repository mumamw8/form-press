import { TNumberField, ZNumberField } from "@/lib/types/form-types"
...
export const NumberField: React.FC<{
  ...
  const { id, label, required, placeholder, helper_text } =
    elementInstance as TNumberField
...
export const NumberFieldDesigner: React.FC<{
  elementInstance: FormElementInstance
}> = ({ elementInstance }) => {
  ...
  return (
    <div className="flex flex-col w-full gap-2">
      ...
      <StyledInput readOnly type="number" placeholder={placeholder} />
...
const propertiesSchema = ZNumberField.omit({
  type: true,
  id: true,
})
type propertiesSchemaType = z.infer<typeof propertiesSchema>
export const NumberFieldProperties: React.FC<{
  elementInstance: FormElementInstance
}> = ({ elementInstance }) => {
  const { updateElement } = useFormBuilderStore((state) => state)
  const element = elementInstance as TNumberField
