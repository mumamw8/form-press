import { TPhoneField, ZPhoneField } from "@/lib/types/form-types"
import {
  FormElementInstance,
  FormElements,
  SubmitFunction,
...
export const PhoneField: React.FC<{
  ...
  const { id, label, required, placeholder, helper_text } =
    elementInstance as TPhoneField
...
export const PhoneFieldDesigner: React.FC<{
  ...
  return (
    <div className="flex flex-col w-full gap-2">
      ...
      <Input readOnly disabled type="tel" placeholder={placeholder} />
      {helper_text && (
...
const propertiesSchema = ZPhoneField.omit({
  type: true,
  id: true,
})
